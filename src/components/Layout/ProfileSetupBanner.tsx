...
export default function ProfileSetupBanner() {
  ...
  return (
    <div
      className={`sticky top-0 z-10 px-4 sm:px-6 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b ${
        isPartial ? 'bg-amber-500/10 border-amber-500/30' : 'bg-rose-500/10 border-rose-500/30'
      }`}
    >
      <div className="flex items-start sm:items-center gap-3">
        <AlertTriangle size={16} className={`shrink-0 mt-0.5 sm:mt-0 ${isPartial ? 'text-amber-400' : 'text-rose-400'}`} />
      ...
      <button onClick={reopenOnboarding} className="btn-secondary shrink-0 text-xs self-start sm:self-auto">Complete setup</button>
    </div>
  );
}
